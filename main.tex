...
\author{
		\name{\bf Marc Harper} \hfill \addr{marcharper@gmail.com}\\
        \addr{Google Inc., Mountain View, CA, USA}
\AND
		\name{\bf Vincent Knight} \hfill \addr{knightva@cardiff.ac.uk}\\
        \addr{Cardiff University, School of Mathematics, UK}
\AND
		\name{\bf Martin Jones} \hfill \addr{martin@pythonforbiologists.com}\\
        \addr{Not affiliated}
\AND
		\name{\bf Georgios Koutsovoulos} \hfill \addr{georgios.koutsovoulos@inra.fr}\\
        \addr{INRA, Université Côte d‘Azur, CNRS, ISA, France}
\AND
		\name{\bf Nikoleta E. Glynatsi} \hfill \addr{glynatsiNE@cardiff.ac.uk}\\
        \addr{Cardiff University, School of Mathematics, UK}
\AND
		\name{\bf Owen Campbell} \hfill \addr{owen.campbell@tanti.org.uk}\\
        \addr{Not affiliated}
}
\date{}
